...
FROM ubuntu:24.04
RUN useradd -r gridvis -u 101 && apt update && apt -y install --no-install-recommends openjdk-17-jre fontconfig fonts-freefont-ttf xvfb libgtk-3-0t64 libxss1 libgbm1 && rm -rf /var/lib/apt/lists/*
